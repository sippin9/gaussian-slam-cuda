...
RasterizeGaussiansCUDA(
    ...
    torch::Tensor out_depth = torch::full({NUM_CHANNELS, H, W}, 0.0, float_opts);
    torch::Tensor out_alpha = torch::full({NUM_CHANNELS, H, W}, 0.0, float_opts);
...
RasterizeGaussiansBackwardCUDA(
    ...
    torch::Tensor dL_ddepths = torch::zeros({P, NUM_CHANNELS}, means3D.options());
    ...
    if (P != 0) {
        CudaRasterizer::Rasterizer::backward(P, degree, M, R,
                                             ...
                                             accum_alphas.contiguous().data_ptr<float>(),
